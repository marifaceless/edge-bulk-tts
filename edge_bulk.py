...
st.header("Voice Selection Tools")
...
with col1:
    ...
    if search_term:
        ...
        st.success(f"Found {len(filtered_voices)} matching voices")
        # Show in a scrollable area to save space
        with st.expander(f"Show all {len(filtered_voices)} matching voices"):
